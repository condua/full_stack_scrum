...
app.post('/login', async (req, res) => {
  ...
  const secret =
    '33e63cdbf2c1b7c12bdef634d08f82bedc42a252963dfade0401af3c354cf3fa';
  // console.log(secret)
      ...
      for (const updatedQuestion of req.body.questions) {
        ...
        if (existingQuestion) {
          ...
          if (
            ...
            Array.isArray(updatedQuestion.options)
          ) {
...
app.get('/exam/:id', async (req, res) => {
  const submits = await Submit.find({ examId: req.params.id });
  const r = [];
  for (let i = 0; i < submits.length; i++) {
    ...
    r.push({
      key: i + 1,
      name: user.fullname,
      score: submits[i].score,
      email: user.email,
      time: submits[i].createdAt,
    });
  }
  console.log(r);
  res.json(r);
...
mongoose
  .connect(
    'mongodb+srv://phanhoangphuc03111:phuc1755@cluster0.b576f71.mongodb.net/API-NODE?retryWrites=true&w=majority',
  )
  .then(() => {
